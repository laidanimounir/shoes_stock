...
class _PurchaseOrdersScreenState extends State<PurchaseOrdersScreen> {
  ...
  Future<void> _confirmOrder(String orderId) async {
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Commande confirmée'), backgroundColor: Color(0xFF4ADE80)));
        _fetchOrders();
      }
    } catch (e) {
  ...
  Future<void> _cancelOrder(String orderId) async {
    try {
      await supabase
          ...
          .eq('id', orderId);
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Commande annulée'), backgroundColor: Color(0xFFFBBF24)));
        _fetchOrders();
      }
    } catch (e) {
      if (mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('$e'), backgroundColor: Color(0xFFF87171)));
  ...
  Future<void> _receiveStock(String orderId, String storeId, List items) async {
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Stock reçu'), backgroundColor: Color(0xFF4ADE80)));
        _fetchOrders();
      }
    } catch (e) {
      if (mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('$e'), backgroundColor: Color(0xFFF87171)));
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    Padding(
                      ...
                      child: Row(
                        children: [
                          const Icon(Icons.add_shopping_cart, color: Color(0xFFF0A500)),
                          ...
                          Row(
                            ...
                            children: [
                              ...
                              Text('${total.toStringAsFixed(2)} ${S.t('misc_currency')}',
                                  style: const TextStyle(fontSize: 18, fontWeight: FontWeight.bold, color: Color(0xFFF0A500))),
                            ],
                          ),
                          const SizedBox(height: 12),
                          SizedBox(
  ...
  Widget _statusChip(String status) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(color: color.withValues(alpha: 0.15), borderRadius: BorderRadius.circular(8), border: Border.all(color: color)),
      child: Row(
        mainAxisSize: MainAxisSize.min,
        children: [
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: Text(S.t('order_title')), backgroundColor: Color(0xFF0A0A14), foregroundColor: Colors.white),
          ...
          : Column(children: [
              ...
              Expanded(child: _filtered.isEmpty
                  ? Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
                      Icon(Icons.receipt_long_outlined, size: 64, color: Color(0xFF2A2A40)),
                  ...
                  return Card(margin: const EdgeInsets.only(bottom: 8), child: Padding(
                    ...
                    child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                      Row(children: [
                        Expanded(child: Text(o['order_number'] ?? '', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 14, color: isCancelled ? Color(0xFF9090A8) : null))),
                        _statusChip(status),
                      ]),
                      const SizedBox(height: 4),
                      Text('${o['supplier_id']?['company_name'] ?? '—'}', style: TextStyle(color: isCancelled ? Color(0xFF606078) : Color(0xFF9090A8), fontSize: 13)),
                      const SizedBox(height: 4),
                      Row(children: [
                        Text('${items.length} ${S.t('order_items')}', style: TextStyle(color: Color(0xFF9090A8), fontSize: 12)),
                        const Spacer(),
                        Text('${o['total_amount'] ?? 0} ${S.t('misc_currency')}', style: TextStyle(fontWeight: FontWeight.bold, color: isCancelled ? Color(0xFF9090A8) : Color(0xFFF0A500))),
                      ]),
                                ...
                                onPressed: () async {
                                  final c = await showDialog<bool>(context: context, builder: (ctx) => AlertDialog(
                                    ...
                                    actions: [
                                      ...
                                      ElevatedButton(onPressed: () => Navigator.pop(ctx, true), style: ElevatedButton.styleFrom(backgroundColor: Color(0xFFF87171), foregroundColor: Colors.white), child: Text(S.t('yes'))),
                                    ],
                                  ));
                            ...
                            if (AppSession.isOwner)
                              Expanded(
                                child: ElevatedButton.icon(
                                  ...
                                  style: ElevatedButton.styleFrom(backgroundColor: Color(0xFF58A6FF), foregroundColor: Colors.white),
                                  onPressed: () async {
                                    final c = await showDialog<bool>(context: context, builder: (ctx) => AlertDialog(
                                      title: Text(S.t('order_confirm')), content: Text(S.t('order_confirm_confirm').replaceAll('{order}', o['order_number'] ?? '')),
                                      actions: [
                                        TextButton(onPressed: () => Navigator.pop(ctx, false), child: Text(S.t('no'))),
                                        ElevatedButton(onPressed: () => Navigator.pop(ctx, true), style: ElevatedButton.styleFrom(backgroundColor: Color(0xFF58A6FF), foregroundColor: Colors.white), child: Text(S.t('yes'))),
                                      ],
                                    ));
                      ...
                      if (status == 'received' && o['received_at'] != null) ...[
                        ...
                        Row(children: [
                          ...
                          Text('Reçue le ${_formatDate(o['received_at'].toString())}', style: TextStyle(color: Colors.green[700], fontSize: 11)),
                        ]),
                      ],
                    ]),
                  ));
                          ...
                          Expanded(
                            child: Column(
                              ...
                              children: [
                                ...
                                Text('Taille: $size  Couleur: $colorInfo', style: TextStyle(color: Color(0xFF9090A8), fontSize: 11)),
