...
**الإصدار الحالي:** v5.3 | **تاريخ آخر تحديث:** 29 مايو 2026  
...
4. [جميع الميزات المكتملة (82)](#جميع-الميزات-المكتملة-82)
...
│  23 Tables  │  44+ RPCs  │  6 Edge Functions  │  RLS       │
└────────────────────────────────────────────────────────────┘
```
...
### الجداول الجديدة المضافة خلال الجلسة (10)
...
| `purchase_price_history` | سجل أسعار الشراء لكل متغير |
...
| `get_analytics_summary` ✅ | ملخص التحليلات مع نسب محسوبة مسبقاً (محدّث — فلتر store_id) |
...
| `get_overdue_customers` ✅ | العملاء المتأخرين في السداد (محدّث — aging buckets) |
...
| `get_admin_dashboard_stats` ✅ | إحصائيات لوحة تحكم المدير (محدّث — فلتر store_id) |
...
| `get_top_products_this_month` ✅ | أفضل منتجات الشهر (محدّث — فلتر store_id) |
| `create_purchase_order` | إنشاء أمر شراء |
| `confirm_purchase_order` | تأكيد أمر الشراء (draft → confirmed) |
| `receive_purchase_order_items` | استلام عناصر أمر الشراء (partial/full) |
| `award_loyalty_points` | منح نقاط ولاء (trigger) |
| `get_end_of_day_report` | تقرير نهاية اليوم (إيرادات/مصاريف/ربح) |
| `get_cashier_session_report` | تقرير جلسة الكاشير لكل موظف |
| `get_unread_notifications` | الإشعارات غير المقروءة |
| `mark_notifications_read` | تعليم الإشعارات كمقروءة |
| `get_price_history` | سجل أسعار الشراء لكل متغير مع رسم بياني |
| `get_supplier_comparison` | مقارنة أسعار الموردين لكل متغير |
| `get_customer_profile` | بطاقة ولاء العميل (Bronze/Silver/Gold) |
| `get_variant_movement_history` | حركة المخزون لكل متغير مع تفاصيل |
| `get_seasonality_report` | تقرير الموسمية (مقارنة شهور عبر السنوات) |
...
### 🛡️ إصلاحات الأخطاء v5.3 (2)
- [x] F1: RPCs now filter by store_id (get_admin_dashboard_stats, get_analytics_summary, get_top_products_this_month)
- [x] F2: Activity logs filtered by store_id for non-owner roles + store_id column in activity_logs table

### 🆕 ميزات جديدة v5.3 (15)
- [x] F3: due_date on invoices with 30-day default for credit sales
- [x] F4: Create Purchase Order form (bon de commande) with supplier + dynamic items
- [x] C1: Complete PO workflow: draft → confirm → partial receive → received → cancel
- [x] C2: Debt aging dashboard with 4 buckets (0-30, 31-60, 61-90, 90+ days) + WhatsApp action
- [x] C3: Per-variant inventory movement history drill-down
- [x] C4: Bulk barcode PDF printing with multi-select
- [x] C5: Inventory export to PDF and Excel (CSV)
- [x] N1: End of day report with revenue/expenses/profit breakdown
- [x] N2: In-app notification system with bell icon badge
- [x] N3: Purchase price history per variant with chart
- [x] N4: Seasonality report comparing months across years
- [x] N5: Supplier price comparison per variant
- [x] N6: Customer loyalty profile card with Bronze/Silver/Gold tiers
- [x] N7: Cashier session report per employee
...
| المقياس | القيمة |
|---|---|
| **إجمالي ملفات Dart** | ~78 ملف (بدون `.g.dart`) |
...
| **v5.3** | تقارير + إشعارات + سجل أسعار + تحسينات الأداء | 29 مايو 2026 | ✅ مكتمل |
...
- هذا README يعكس **ما تم تنفيذه فعلياً** — 82 مهمة مكتملة، 81 commit.
